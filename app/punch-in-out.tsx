...
export default function PunchInOut() {
  ...
  const handlePunchIn = async () => {
    if (!locationPermission) {
      Alert.alert(
        "Location Required",
        "Please enable location permission to punch in/out",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Punched In",
        `Successfully punched in at ${newPunch.punchIn}`,
      );
    } catch (error) {
  ...
  const handlePunchOut = async () => {
    ...
    try {
      ...
      const totalHours =
        (punchOutTimeObj.getTime() - punchInTime.getTime()) / (1000 * 60 * 60);
      ...
      Alert.alert(
        "Punched Out",
        `Successfully punched out at ${punchOutTime}. Total hours: ${completedPunch.totalHours}`,
      );
    } catch (error) {
  ...
  const getTotalHoursToday = () => {
    ...
    return todayPunches.reduce(
      (total, punch) => total + (punch.totalHours || 0),
      0,
    );
  };
    ...
    <View style={styles.container}>
        ...
        {currentPunch ? (
          <View style={styles.activeStatus}>
            ...
            <Text style={styles.punchTime}>
              Punched in at: {currentPunch.punchIn}
            </Text>
            <Text style={styles.locationText}>
              📍 {currentPunch.location.address}
            </Text>
          </View>
        ) : (
      ...
      <View style={styles.statsCard}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statNumber}>
              {getTotalHoursThisWeek().toFixed(1)}
            </Text>
            ...
            <View key={punch.id} style={styles.historyItem}>
              <View style={styles.historyHeader}>
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        punch.status === "completed" ? "#10b981" : "#f59e0b",
                    },
                  ]}
                >
              ...
              <View style={styles.historyDetails}>
                <Text style={styles.historyTime}>
                  In: {punch.punchIn}{" "}
                  {punch.punchOut && `| Out: ${punch.punchOut}`}
                </Text>
                {punch.totalHours && (
